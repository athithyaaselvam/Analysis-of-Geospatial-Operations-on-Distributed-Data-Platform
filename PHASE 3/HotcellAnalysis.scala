...
def calculateGetisOrd(wijxj:Int, totalNeighbours:Int, stddev:Double, numCells:Int, mean:Double):Double = {

	val numerator = wijxj - mean*26
	val denominator = stddev*math.sqrt( (numCells*26 - (26*26)  )/ numCells-1 ) 
	if (denominator == 0.0){
		return 0.0
	}
...
def runHotcellAnalysis(spark: SparkSession, pointPath: String): DataFrame =
{
  ...
  val totalPickups =  pickupInfo.agg(sum("count")).first.getLong(0)
  val pickupList = pickupInfo.collect()
  ...
  for (row <- pickupList) {
    ...
    countSquare += row.getLong(3)*row.getLong(3)
  }

  val mean = totalPickups.toDouble/numCells.toDouble
  val stddev = Math.sqrt(countSquare.toDouble/numCells.toDouble - mean*mean)

  spark.udf.register("calculateGetisOrd", (wijxj:Int, totalNeighbours:Int) => (calculateGetisOrd(wijxj, totalNeighbours, stddev, numCells.toInt, mean)) )
